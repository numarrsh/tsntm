...
class DoublyHelper(Helper):
  """A helper for use during training.  Only reads inputs.

  Returned sample_ids are the argmax of the RNN output logits.
  """

  def __init__(self, inputs, sequence_length, initial_state, init_lengths, time_major=False, name=None):
    """Initializer.
    ...
    Raises:
      ValueError: if `sequence_length` is not a 1D tensor.
    """
    with ops.name_scope(name, "DoublyHelper", [inputs, sequence_length]):
      inputs = ops.convert_to_tensor(inputs, name="inputs")
      self._inputs = inputs
      if not time_major:
        inputs = nest.map_structure(_transpose_batch_time, inputs)
      ...
      self._initial_state = ops.convert_to_tensor(initial_state, name="initial_state")
      self._init_lengths = ops.convert_to_tensor(init_lengths, name="init_lengths")
  ...
  def next_inputs(self, time, outputs, state, name=None, **unused_kwargs):
    ...
    with ops.name_scope(name, "TrainingHelperNextInputs",
      ...
      initialize = math_ops.reduce_any(math_ops.equal(next_time, self._init_lengths))
      state = control_flow_ops.cond(
          initialize, lambda: self._initial_state,
          lambda: state)
      return (finished, next_inputs, state)
